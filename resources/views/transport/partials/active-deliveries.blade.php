    ...
    <div class="card p-4 rounded-4 shadow-sm border-translucent bg-body">
        <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3 mb-4 border-bottom pb-3">
            <div>
                ...
                <p class="text-muted small mb-0 mt-1">Live tracking of ongoing deliveries currently assigned, dispatched, or in transit.</p>
            </div>
            <span class="badge bg-primary-subtle text-primary border border-primary-subtle rounded-pill px-3 py-2 fw-bold">
                {{ $requests->total() }} Active Deliveries
            </span>
        </div>
                ...
                <tbody>
                    @forelse($requests as $req)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('transport.index', ['tab' => 'delivery-orders', 'task_id' => $req->id]) }}" class="btn btn-sm btn-outline-primary rounded-pill px-3">
                                    View Details &rarr;
                                </a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="7" class="text-center text-muted py-5">
                                ...
                                <div>No active deliveries in transit.</div>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
