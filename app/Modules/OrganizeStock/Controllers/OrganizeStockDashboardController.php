...
class OrganizeStockDashboardController extends Controller
{
    public function index(): View
    {
        $kpis = [
            ...
            'pending_dispatch' => DispatchTask::whereIn('status', ['pending_pack', 'packed'])->count(),
        ...
        $recentDispatches = DispatchTask::latest()->take(5)->get();
        $recentExceptions = WarehouseException::with('product')->latest()->take(5)->get();

        return view('organize.dashboard', compact('kpis', 'warehouses', 'recentPutaways', 'recentPicks', 'recentDispatches', 'recentExceptions'));
    }
}
